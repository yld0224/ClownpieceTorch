...
## tutorial 中的问题
**为什么多线程时不能用 `while queue is not empty` 作为退出条件？**
当前任务队列是空的并不等于所有任务都已经做完了,很可能只是某个时刻某个线程刚好把队列中的任务取走,此时不能退出.正确的退出条件是未完成的task的计数归0.

**为什么即使算子层面有多线程，反向执行的多线程仍然重要？**
这是不同层次的多线程,算子层面的多线程是对于某个特定tasknode中的计算加速.反向执行的多线程是对于计算图的整体计算加速,多线程负责并发计算不同的tasknode,进一步加速计算.
